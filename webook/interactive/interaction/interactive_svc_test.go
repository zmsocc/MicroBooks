...
func (s *InteractiveTestSuite) TestIncrReadCnt() {
	testCases := []struct {
		...
		bizId   int64
		wantErr error
		wantRes *intrv1.IncrReadCntResponse
	}{
		{
			// DB 和缓存都有数据
			name: "增加成功,db和redis",
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before(t)
			res, err := svc.IncrReadCnt(context.Background(), &intrv1.IncrReadCntRequest{
				Biz:   tc.biz,
				BizId: tc.bizId,
			})
...
func (s *InteractiveTestSuite) TestLike() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.before(t)
			res, err := svc.Like(context.Background(), &intrv1.LikeRequest{
				Biz:   tc.biz,
				BizId: tc.bizId,
				Uid:   tc.uid,
			})
...
func (s *InteractiveTestSuite) TestDislike() {
	...
	testCases := []struct {
		...
		wantErr error
		wantRes *intrv1.CancelLikeResponse
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.before(t)
			res, err := svc.CancelLike(context.Background(), &intrv1.CancelLikeRequest{
				Biz:   tc.biz,
				BizId: tc.bizId,
				Uid:   tc.uid,
			})
...
func (s *InteractiveTestSuite) TestCollect() {
	testCases := []struct {
		...
		wantErr error
		wantRes *intrv1.CollectResponse
	}{
		{
			name:   "收藏成功,db和缓存都没有",
			before: func(t *testing.T) {},
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before(t)
			res, err := svc.Collect(context.Background(), &intrv1.CollectRequest{
				Biz:   tc.biz,
				BizId: tc.bizId,
				Cid:   tc.cid,
				Uid:   tc.uid,
